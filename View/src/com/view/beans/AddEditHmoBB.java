...
//                        dep.equals("Parent") || dep.equals("Domestic Staff") || dep.equals("Child")) {
                     if (dep.equals("Sibling") || dep.equals("Grand Parent") ||
                        dep.equals("Parent") || dep.equals("Domestic Staff") || dep.equals("Child")) {
                        h++;
                        System.err.println("Came inside EMP + spouse");
...
//            if (h > 1 || i > 0) {
              if (h > 0 || i > 0) {
                System.err.println("Please delete all the dependent type except only one spouse ");
    ...
    public Boolean getMaxlimit(){
            Boolean limit = true;
           String userName = (String)JSFUtils.resolveExpression("#{sessionScope.userName}");
            ViewObject limitVo = ADFUtils.findIterator("Maxlimit_vo1Iterator").getViewObject();
            ViewCriteria viewCriteria = limitVo.createViewCriteria();
            ViewCriteriaRow viewCriteriaRow = viewCriteria.createViewCriteriaRow();
            viewCriteriaRow.setAttribute("Username", userName);
            
            viewCriteria.addRow(viewCriteriaRow);
            limitVo.applyViewCriteria(viewCriteria);
            limitVo.executeQuery();
            
            if(limitVo.getEstimatedRowCount()>0)
            {    
                limit= false;
            }
            return limit;
